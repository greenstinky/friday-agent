...
if not creds or not creds.valid:
    if creds and creds.expired and creds.refresh_token:
        ...
        try:
            ...
            # Create a flow for device authorization
            flow = Flow.from_client_secrets_file(
                'credentials.json',
                scopes=SCOPES,
                redirect_uri='urn:ietf:wg:oauth:2.0:oob'  # This will be ignored in device flow
            )

            # Use device flow
            flow = flow.device_flow()
            device_code_response = flow.fetch_device_code()

            print(f"Please visit this URL to authenticate: {device_code_response['verification_url']}")
            ...
            creds = flow.fetch_token(device_code=device_code_response['device_code'])
