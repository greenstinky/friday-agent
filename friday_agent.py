...
try:
    ...
    drive_service = build('drive', 'v3', credentials=credentials, requestBuilder=lambda *args, **kwargs: HttpRequest(NoSSLVerifyTransport(), *args, **kwargs))
except Exception as e:
    print(f"Failed to authenticate with Google Drive using service account: {e}")
    sys.exit(1)
...
RECORD_SECONDS = 15  # Increased for better speech recognition
SILENCE_DURATION = 0.5
SILENCE_THRESHOLD = 80  # Adjusted for better sensitivity
...
# Text-to-speech with terminal output
def speak(text):
    print(f"Friday says: {text}")  # Print what Friday is saying
    audio_queue.put(text)

# Speech recognition setup
model = Model("/home/eric/vosk-model-small-en-us-0.15")
...
def record_audio():
    ...
    try:
        ...
        print("Listening... Speak now! (Up to 15 seconds, stops on silence)")
        frames = []
        silent_chunks = 0
        ...
        for i in range(max_chunks):
            ...
            if signal.getsignal(signal.SIGINT) != signal_handler:
                print("Interrupt detected during recording.")
                break
        ...
        # Save the raw audio
        wf = wave.open("raw_input.wav", 'wb')
        wf.setnchannels(CHANNELS)
        wf.setsampwidth(p.get_sample_size(FORMAT))
        wf.setframerate(RATE)
        wf.writeframes(b''.join(frames))
        wf.close()
        # Apply noise suppression using sox
        subprocess.run(["sox", "raw_input.wav", WAV_FILE, "noisered", "noise.wav", "0.21"], check=True)
        os.remove("raw_input.wav")
        return WAV_FILE
    except Exception as e:
        print(f"Error during audio recording: {e}")
        log_message(f"Error during audio recording: {e}")
        return None
    finally:
        if stream:
            stream.stop_stream()
...
# File to store the FridayLogs folder ID
FOLDER_ID_FILE = "fridaylogs_folder_id.txt"

# Upload logs to Google Drive with retry logic and persistent folder ID
def upload_logs_to_drive():
    max_attempts = 3
    folder_id = None

    # Try to load the folder ID from the file
    if os.path.exists(FOLDER_ID_FILE):
        with open(FOLDER_ID_FILE, "r") as f:
            folder_id = f.read().strip()
            print(f"Loaded FridayLogs folder ID from file: {folder_id}")

    for attempt in range(max_attempts):
        try:
            # Search for the "FridayLogs" folder
            folder_name = "FridayLogs"
            query = f"name='{folder_name}' and mimeType='application/vnd.google-apps.folder' and trashed=false"
            response = drive_service.files().list(q=query, spaces='drive', fields='files(id)').execute()
            folders = response.get('files', [])

            # If the folder ID was loaded but not found, clear it
            if folder_id and not any(folder['id'] == folder_id for folder in folders):
                print(f"Stored folder ID {folder_id} not found. Clearing it.")
                folder_id = None
                if os.path.exists(FOLDER_ID_FILE):
            ...
            if not folders:
                ...
                print(f"Created 'FridayLogs' folder with ID: {folder_id}")

                # Share the folder with ericjavery@gmail.com
                permission = {
                    'type': 'user',
                    'role': 'writer',
                    'emailAddress': 'ericjavery@gmail.com'
                }
                drive_service.permissions().create(fileId=folder_id, body=permission, fields='id').execute()
                print(f"Shared 'FridayLogs' folder with ericjavery@gmail.com")

                # Store the folder ID
                with open(FOLDER_ID_FILE, "w") as f:
                    f.write(folder_id)
                print(f"Stored FridayLogs folder ID: {folder_id}")
            else:
                # Use the stored folder ID if available, otherwise pick the first one
                if folder_id:
                    folder_id = next((folder['id'] for folder in folders if folder['id'] == folder_id), folders[0]['id'])
                else:
                    ...
                    with open(FOLDER_ID_FILE, "w") as f:
                        f.write(folder_id)
                    print(f"Stored FridayLogs folder ID: {folder_id}")
                print(f"Found 'FridayLogs' folder with ID: {folder_id}")

            # Upload the log file to the "FridayLogs" folder
            file_metadata = {
                'name': os.path.basename(LOG_FILE),
                'parents': [folder_id]
            }
            media = MediaFileUpload(LOG_FILE)
            file = drive_service.files().list(q=f"name='{os.path.basename(LOG_FILE)}' and '{folder_id}' in parents and trashed=false", fields='files(id)').execute()
            if file.get('files'):
            ...
            print(f"Failed to upload logs to Google Drive (attempt {attempt + 1}/{max_attempts}): {e}")
            log_message(f"Failed to upload logs to Google Drive: {e}")
            if attempt < max_attempts - 1:
                time.sleep(5)  # Wait 5 seconds before retrying
            else:
                print("All attempts to upload logs failed. Logs will be stored locally.")
...
def setup_github_repository():
    ...
    try:
        ...
        speak(f"{sassy_prefix} I couldn’t set up my GitHub repository, honey: {str(e)}")
        print(f"Failed to set up GitHub repository: {str(e)}")
        return False
        ...
        while True:
            ...
            if audio_file is None:
                ...
                if retry_count >= max_retries:
                    speak("I’m having trouble recording audio after several attempts. I’ll stop listening for now. Please restart the script to try again.")
                    print("Maximum retries reached. Exiting listening loop.")
                    break
                else:
                    speak("I had trouble recording audio. Let’s try again—please speak clearly!")
                continue
            ...
            if user_input in ["sorry, i didn’t catch that", "oops, audio format issue"]:
                retry_count += 1
                if retry_count >= max_retries:
                    speak("I’m having trouble hearing you after several attempts. I’ll stop listening for now. Please restart the script to try again.")
                    print("Maximum retries reached. Exiting listening loop.")
                    break
                else:
