...
if not creds or not creds.valid:
    if creds and creds.expired and creds.refresh_token:
        ...
        try:
            ...
            # Generate the authentication URL without specifying redirect_uri here
            auth_url, _ = flow.authorization_url(prompt='consent')
            print(f"Please go to this URL to authenticate: {auth_url}")
            print("After authenticating, you will receive a code. Copy the code and paste it below.")
            code = input("Enter the authentication code: ")
            # Specify the redirect_uri only in fetch_token
